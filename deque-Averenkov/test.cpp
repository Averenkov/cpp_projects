...
TestGroup create_iterator_tests() {
    return { "iterators",
        ...
        make_test<PrettyTest>("arithmetic", [](auto& test){
            ...
            // Note, that even incrementing end() iterator
            // is considered UB
            Deque<int> one(1);
            auto iter = one.begin();
            test.equals(iter++, one.begin());
            auto iter2 = one.end();
            ...
            auto r_iter = one.rbegin();
            test.check((r_iter++ == one.rbegin()));
            test.check((empty.cend() - empty.cbegin()) == 0);
            test.check((empty.cbegin() + 0 == empty.cend()) && (empty.cend() - 0 == empty.cbegin()));
            auto c_iter = one.cbegin();
            test.check((c_iter++ == one.cbegin()));
...
TestGroup create_modification_tests() {
    return { "Modification",
        ...
        make_test<PrettyTest>("insert and erase", [](auto& test){
            ...
            test.equals(d[start_size / 2].data, 2);
            test.equals(d[start_size / 2 - 1].data, 1);
            test.equals(d[start_size / 2 + 1].data, 1);
